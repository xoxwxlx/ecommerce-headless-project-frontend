...
export default function BooksGenrePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    async function fetchBooks() {
      try {
        ...
        // Filtruj książki fizyczne i według gatunku
        const filtered = products.filter((p: Product) => {
          const isBook = p.format === 'physical' || p.format === 'both' || p.format === 'paperback';
          const matchesGenre = p.genre?.toLowerCase() === apiGenre?.toLowerCase();
          return isBook && matchesGenre;
        });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#F0F4FF] via-white to-[#FFF8DE]">
      ...
      <div className="container mx-auto px-4 py-8">
        <nav className="mb-6">
          <Link href="/books" className="text-[#8CA9FF] hover:text-[#AAC4F5] transition-colors">
            ← Powrót do książek
          </Link>
        </nav>
        ...
        <h1 className="text-4xl font-bold mb-8">
          ...
          <span className="text-gray-800"> - Książki</span>
        </h1>
            ...
            <main className="flex-1">
              ...
              {filteredProducts.length > 0 ? (
                ...
                <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
                  <p className="text-gray-500">Brak dostępnych książek spełniających wybrane kryteria</p>
                  {searchQuery && (
                    <button
